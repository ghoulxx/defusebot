...
module.exports = {
  ...
  async execute(client, channel) {
    ...
    if (!shouldTriggerAntiNuke(channel.guild.id, 'channelCreate', antiNukeState)) return;

    const executor = await getExecutorFromAudit(channel.guild, AuditLogEvent.ChannelCreate);
    const punishment = config.punishments?.massChannelCreate || 'kick';
    await executePunishment(channel.guild, executor, punishment, 'Anti-nuke triggered: rapid channel creation');
  }
};
