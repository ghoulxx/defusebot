...
module.exports = {
  ...
  async execute(client, role) {
    ...
    if (!shouldTriggerAntiNuke(role.guild.id, 'roleDelete', antiNukeState)) return;

    const executor = await getExecutorFromAudit(role.guild, AuditLogEvent.RoleDelete);
    if (!executor || isTrustedTarget(config, executor)) return;

    const punishment = config.punishments?.massRoleDelete || 'kick';
    await executePunishment(role.guild, executor, punishment, 'Anti-nuke triggered: rapid role deletion');
  }
};
